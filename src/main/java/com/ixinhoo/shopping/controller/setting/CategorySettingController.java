...
import com.ixinhoo.shopping.service.setting.CategorySettingService;
 ...
 */
@RestController
@RequestMapping(value = "/api/bg/category-setting")
public class CategorySettingController {
    ...
    public StatusDto save(CategorySetting reqDto) {
        return service.saveCategorySetting(reqDto);
    }


    @RequestMapping(value = "show/{id}", method = RequestMethod.GET)
    public DetailDto<CategorySetting> findById(@PathVariable("id") Long id) {
        return new DetailDto<>(true, BeanMapper.map(service.findById(id), CategorySetting.class));
    }
    ...
    public DataTable<CategorySetting> list(HttpServletRequest request) {
        return service.listCategorySetting(DataTableUtil.toDataTable(request));
    }
}